...
export function PlanCardEditable({
  ...
  const [localTopicos, setLocalTopicos] = useState<string[]>(
  ...
  const [inputPrecoAntigo, setInputPrecoAntigo] = useState<string>(
    plan.preco_antigo?.toString(),
  )
  const [inputTipoPagamento, setInputTipoPagamento] = useState<
    string | undefined
  >(plan.tipo_pagamento || '')
  const [inputDuracaoPlanoEmDias, setInputDuracaoPlanoEmDias] =
    useState<string>(plan.duracao_plano_em_dias?.toString())
  const [inputIdPrecoStripe, setInputIdPrecoStripe] = useState<
    string | undefined
  ...
  const handleEditPlan = async () => {
    const editRequest: EditPlanRequest = {
      ...
      preco_antigo:
        inputPrecoAntigo !== null && inputPrecoAntigo !== ''
          ? parseFloat(inputPrecoAntigo)
          : null,
      tipo_pagamento: inputTipoPagamento || null,
      duracao_plano_em_dias:
        inputDuracaoPlanoEmDias !== null && inputDuracaoPlanoEmDias !== ''
      ...
      <Styled.Content>
        ...
        <Styled.PlanDetailsContainer $expanded={isExpanded}>
          ...
          {isAdmin && visibleDetails.length === 0 && (
            <Text text="adicionar topicos" size={Size.S} center />
          )}
          {isAdmin && (
            <Icon
        ...
        />
        {plan.preco !== 0 && <Styled.Button>Assinar</Styled.Button>}
      </Styled.Content>
    </Styled.AllContent>
  )
}
